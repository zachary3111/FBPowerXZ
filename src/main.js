...
try {
  ...
  const crawler = new PlaywrightCrawler({
    ...
    launchContext: { launchOptions: { headless: true, args: ['--no-sandbox','--disable-dev-shm-usage'] } },
      ...
      if (session && cookies.length && !session.get('cookiesSet')) {
        try {
          const normalized = cookies.map(c => ({
            name: c.name,
            value: c.value,
            domain: c.domain && c.domain.startsWith ? (c.domain.startsWith('.') ? c.domain : c.domain) : 'm.facebook.com',
            path: c.path || '/',
  ...
  await Actor.pushData({ _summary: { query, total, maxResults, start_date: input.start_date || null, end_date: input.end_date || null, recent_posts: recentOnly } });
  log.info('Done.');
} catch (err) {
